...
namespace syncfusion.richtextboxdemos.wpf
{
    ...
    public partial class GrammarDemo : ChromelessWindow
    {
        ...
        private void UpdateDocumentBackground()
        {
            ...
            {
                string typeName = string.Format("Syncfusion.Themes.{0}.WPF.{0}SkinHelper, Syncfusion.Themes.{0}.WPF", themeName);
                themeType = Type.GetType(typeName, false);
            }

            // Fallback to the default system theme type if resolution failed.
            if (themeType == null)
                themeType = typeof(SystemThemeSkinHelper);

            ThemeKey themeKey = new ThemeKey { Theme = themeType, Key = "ContentBackground" };
            SolidColorBrush brush = TryFindResource(themeKey) as SolidColorBrush;
            richTextBoxAdv.Document.Background.Color = (brush?.Color) ?? Colors.White;
            showParagraphRTE.Document.Background.Color = (brush?.Color) ?? Colors.White;
        }
        #endregion
