...
namespace syncfusion.richtextboxdemos.wpf
{
    ...
    public partial class CreateLettersDemo : RibbonWindow
    {
        ...
        private void InitializeReportTab()
        {
            ...
            IEnumerable<Visual> childRibbon = VisualUtils.EnumChildrenOfType(richTextRibbon, typeof(Ribbon));
            if (childRibbon == null || childRibbon.Count() == 0)
                return;
            ribbon = childRibbon.First() as Ribbon;
            if (ribbon != null)
            {
                ...
                if (reportsTab != null)
                {
                    recipients = (reportsTab.Items[0] as RibbonBar).Items[1] as RibbonComboBox;
                }
            }
        }

        /// <summary>
        /// Called on Restrict editing option is clicked.
        /// </summary>
        private void PART_RestrictEditingButton_Click(object sender, RoutedEventArgs e)
        {
            if ((sender as RibbonButton).IsEnabled)
                EnableReportsTab();
        }
        /// <summary>
        /// Enables the reports tab in SfRichTextRibbon.
        /// </summary>
        private void EnableReportsTab()
        {
            // Reports bar alone should be enabled on read-only mode.
            if (reportsTab != null && reportsTab.Items.Count > 0 && reportsTab.Items[0] is RibbonBar reportsBar)
                reportsBar.IsEnabled = true;
        }
        #endregion
